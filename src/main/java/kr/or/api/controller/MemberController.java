...
public class MemberController {
	...
	public String sigUpCheck(MemberVO memberVO, Model model) {
		...
		if (StringUtils.isEmpty(memberVO.getMemId()) || StringUtils.isEmpty(memberVO.getMemPw())
				|| StringUtils.isEmpty(memberVO.getMemPhone()) || StringUtils.isEmpty(memberVO.getMemName())
				|| StringUtils.isEmpty(memberVO.getMemEmail()) || StringUtils.isEmpty(memberVO.getMemGender())
	...
	@RequestMapping(value = "/chat/chatList.do", method = RequestMethod.POST)
	public String signInCheck(MemberVO memberVO, Model model, HttpSession session) {
		...
		if(check==1) {
			...
			return "redirect:/chat/chatListView.do";
		} else {
	 ...
	 * String signInCheck(MemberVO memberVO, Model model, HttpSession session) {
	 * 
	 * 
	 * log.info(memberVO.getMemId()); log.info(memberVO.getMemPw());
	 * 
	 * int check = service.memLogin(memberVO);
	 * 
	 * if(check==1) { session.setAttribute("memberVO", memberVO); return
	 * "/pages/chat"; }else { model.addAttribute("flag","no"); return
	 * "pages/ddit_signin";
	 * 
	 * }
	 * 
